...
void mapping(const YAML::Node& node, const std::string& result_path, const std::string& lpips_path)
{
    ...
    // Initialize coarse-to-fine manager if enabled
    if (prm.enable_coarse_to_fine) {
        CoarseToFineParams ctf_params;
        ctf_params.initial_sampling_ratio = prm.initial_sampling_ratio;
        ctf_params.min_initial_points = prm.min_initial_points;
        ctf_params.max_initial_points = prm.max_initial_points;
        ctf_params.densify_grad_threshold = prm.densify_grad_threshold;
        ctf_params.densify_size_threshold = prm.densify_size_threshold;
        ctf_params.densify_start_iter = prm.densify_start_iter;
        ctf_params.densify_stop_iter = prm.densify_stop_iter;
        ctf_params.densify_interval = prm.densify_interval;
        ctf_params.prune_opacity_threshold = prm.prune_opacity_threshold;
        ctf_params.prune_scale_threshold = prm.prune_scale_threshold;
        ctf_params.prune_start_iter = prm.prune_start_iter;
        ctf_params.prune_interval = prm.prune_interval;
        
        initializeCoarseToFineManager(ctf_params);
        std::cout << "\033[1;32m[CoarseToFine] Manager initialized successfully\033[0m" << std::endl;
    }
    ...
    while (!exit_flag)
    {
        ...
        if (!gaussians->is_init_)
        {
            ...
            // Use coarse-to-fine initialization if enabled
            if (prm.enable_coarse_to_fine) {
                gaussians->initializeCoarseToFine(dataset);
            } else {
        ...
        /// [5] optimize map
        t_start = std::chrono::steady_clock::now();
        double updated_num = optimize(dataset, gaussians, iteration_count);
        torch::cuda::synchronize();
        t_end = std::chrono::steady_clock::now();
