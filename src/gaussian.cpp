...
void GaussianModel::initializeCoarseToFine(const std::shared_ptr<Dataset>& dataset)
{
    if (getCoarseToFineManager()) {
        auto self = shared_from_this();
        getCoarseToFineManager()->initializeCoarse(self, dataset);
    } else {
...
double optimize(const std::shared_ptr<Dataset>& dataset, std::shared_ptr<GaussianModel>& pc, int iteration)
{
    ...
    // Apply coarse-to-fine strategy if enabled
    if (getCoarseToFineManager()) {
        // Note: updateRefinementLevel() removed in SplaTAM-style implementation
        // SplaTAM uses one-time initialization + silhouette-guided densification
        getCoarseToFineManager()->adaptiveDensification(pc, iteration);
        getCoarseToFineManager()->pruneGaussians(pc, iteration);
    }
